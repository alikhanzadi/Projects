...
#TO DO: add credential file support as a param for the function call.
    ...
    with open('./lib/Database_credentials.yml', 'r') as f:
...
def select_statement ( SQLstr, location = 'remote'):
    """
    This function will return rows from the given SQL statement.
    """
    
    try:
        connection, cursor = connect_to_postgres(location)
        returned_pages = pd.read_sql(SQLstr, con=connection)
        cursor.close()
        connection.close()
        return returned_pages
    except psycopg2.Error as e:
